...
import cusdIcon from "../assets/cusd.png";
...
export default class ListingDetails extends Component {
  ...
  sendCUSD = async (_payTheMan) => {
    ...
    const url =
      "https://api.coingecko.com/api/v3/simple/price?ids=cusd&vs_currencies=USD";
    var rate;
    await fetch(url, post)
      .then((resp) => resp.json())
      .then((data) => (rate = data));
    const rateUSD = rate.cusd.usd;
    ...
    if (window.ethereum) {
      ...
      const contractCUSD = new window.web3.eth.Contract(
        contractABICUSD,
        contractAddressCUSD
      );
      console.log(contractCUSD);
  ...
  sendTestnetCUSD = async (_shippingAddress) => {
    // Get addresses
                  ...
                  <div
                    ...
                    <Row style={{ marginTop: "15px" }}>
                      <Image
                        src={cusdIcon}
                        roundedCircle
                        style={styles.cusdIcon}
                      />
                      <p style={styles.paymentInfo}>All payments in CUSD</p>
                    </Row>
                  </div>
